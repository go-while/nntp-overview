...
func Rescan_Overview(file_path string, group string, mode int, DEBUG bool) (bool, uint64) {
    ...
    if DEBUG_OV { log.Printf(" -> Start Rescan_Overview: fp='%s' group='%s' mode=%d", file_path, group, mode) }
    ...
    len_mmap := uint64(len(mmap_handle))
    ...
    endindex := len_mmap-1
    if DEBUG_OV { log.Printf(" --> rescan_OV mode=%d len=%d startindex=%d", mode, len_mmap, startindex) }
    ...
    if mode == 2 { return true, 0 }

    line, lines, newlines, tabs, frees, position := "", uint64(0), 0, 0, 0, uint64(OV_RESERVE_BEG)
    ...
    for i := startindex; i <= endindex; i++ {
        ...
        switch(c) { // check what this byte char is
            ...
            case '\n':
                ...
                if frees > 0 {
                    ...
                    if rem[len(rem)-2] == 0 && rem[len(rem)-1] == ',' && eof == FOOTER_END {
                        ...
                        if len(foot) != SIZEOF_FOOT {
                            log.Printf("ERROR Rescan_OV foot=%d != %d fp='%s'", foot, SIZEOF_FOOT, filepath.Base(file_path))
                            return false, 0
                        }
                        ...
                        f_indx := utils.Str2uint64(foot_index[1])
                        f_bodyend := utils.Str2uint64(foot_bodyend[1])
                        f_fend := utils.Str2uint64(foot_fend[1])
                        ...
                        if DEBUG_OV { log.Printf(" --> Rescan_OV footer f_time=%d age=%d fp='%s'", f_time/1e9, last_modified, filepath.Base(file_path)) }
                        ...
                        if DEBUG_OV { log.Printf(" --> Rescan_OV footer last=OK=%d  fp='%s'", f_last, filepath.Base(file_path)) }
                        ...
                        if DEBUG_OV { log.Printf(" --> Rescan_OV footer Findex=OK=%d fp='%s'", f_indx, filepath.Base(file_path)) }

                        if f_fend-f_bodyend != uint64(OV_RESERVE_END) {
                        ...
                        if DEBUG_OV { log.Printf(" --> Rescan_OV footer f_zero=%d fp='%s'", f_zero, filepath.Base(file_path)) }
                ...
                if position == uint64(OV_RESERVE_BEG) && tabs == 0 {
                    newlines = 0
                    position++
                ...
                if tabs == OVERVIEW_TABS {
                    ...
                    // be verbose?
                    if DEBUG_OV { log.Printf(" ----> Rescan: line=%d msgnum=%d tabs=%d newslines=%d len=%d pos=%d last_newline_pos=%d" , lines, msgnum, tabs, newlines, len(line), position, last_newline_pos) }
                    last_msgnum = msgnum
                    //last_line = line
                } else {
                ...
                if mode == 3 || mode == 997 {
                    last_line, last_newlines, last_tabs, last_beg = line, newlines, tabs, position-uint64(len(line)) // capture
                ...
                if tabs == OVERVIEW_TABS && mode >= 998 {
                    ...
                    */
                    ...
                    if !isvalidmsgid(msgid, false) {
                        log.Printf("ERROR Rescan_OV#5 @line=%d !isvalidmsgid", lines, msgnum)
                        return false, 0
                    }
                    ...
                    if bytes <= 0 && deezlines <= 0 {
                        log.Printf("ERROR Rescan_OV#6 @line=%d bytes=%d lines=%d", lines, msgnum, bytes, deezlines)
                        return false, 0
                    }
    ...
    badfooter := false
    var gibb uint64
    toend := uint64(len_mmap) - position
    last_end := last_beg+uint64(len(last_line))

    if toend != uint64(OV_RESERVE_END) {
        badfooter = true
    }
    ...
    if badfooter {
        ...
        if newlines == 0 || tabs != OVERVIEW_TABS {
            gibb = uint64(len_mmap-1) - last_end
    ...
    if mode < 999 { return true, last_msgnum }
    ...
    if mode == 999 && badfooter && fix_flag == "fix-footer" {
        ...
        ovfh.Mmap_size = uint64(len_mmap)
